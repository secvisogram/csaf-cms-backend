...
| body
| An advisory in CSAF JSON format including comments. <<string>>
| X
| 
| 
...
Add a comment to the advisory with the given ID. The comments are generated independently of the CSAF document. The IDs of the comments must be added manually to the appropriate place in the CSAF document and then saved with the document.
...
| advisoryId
| The ID of the advisory to add the comment to. 
| X
| null
| 
...
Create a new CSAF document with optional comments in the system.
...
Delete a CSAF document from the system. All older versions, comments and audit-trails are also deleted.
...
| 
| String 
| A comment is added to an object in the CSAF document. This name specifies the concrete value and its path in the object the comment belongs to. When it is empty, the comment belongs to the whole object.
|  
...
| 
| Long 
| Only present if the comment is an answer. The ID of the comment the answer belongs to.
| int64 
...
| 
| String 
| A comment is added to an object in the CSAF document. This name specifies the concrete value and its path in the object the comment belongs to. When its empty, the comment belongs to the whole object.
|  
